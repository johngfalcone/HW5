...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/falconeHW5"
## Provided:
...
class UpdateButtonForm(FlaskForm):
    update = SubmitField("Update your to-do items")
...
class UpdatePriorityForm(FlaskForm):
    update_priority = StringField("What is the new priority of this to-do item?", validators=[Required()])
    update = SubmitField("Update item priority")
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        changed_priority = form.update_priority.data
        active_item = TodoItem.query.filter_by(description = item).first()
        active_item.priority = changed_priority
...
def delete(lst):
    ...
    active_list = TodoList.query.filter_by(title = lst).first()
    db.session.delete(active_list)
    db.session.commit()

    flash("Deleted list {}".format(active_list.title))
    return redirect(url_for("all_lists"))
